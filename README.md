...
> 适合：安全演示、流量回溯分析、检测规则原型验证、教学/课程项目。
...
- **PCAP 解析**：dpkt + scapy（项目依赖中同时包含，解析实现以 parsers 为准）
- **数据分析**：pandas + numpy
...
- requirements.txt 中包含 **Celery + Redis**，适合将导入/检测改造成真正的异步队列任务（当前实现以 FastAPI BackgroundTasks 为主）。
...
> 当前鉴权为“前端保护 + 后端 token 校验接口”的轻量实现，便于演示。
...
- 启动时自动检查并写入演示数据集（避免首次进入系统空白）
- 默认生成约 100 条 Flow，并包含少量 DNS/HTTP/TLS 事件与告警样例
...
- 当前登录模块为演示实现：账号/密码在代码内固定，token 存在内存字典中（服务重启会失效）。
- 目前主要用于“前端入口保护”，API 本身并未对所有业务路由强制鉴权（如需可在后端为路由加 `Depends(get_current_user)` 或增加中间件）。

## 常见问题

1. **首次进入没数据？**
     - 服务启动会自动写入一个“演示数据集”。如果已存在同名/同文件名演示数据，则不会重复写入。

2. **上传后一直 pending/running？**
     - 检查导入后台任务是否报错；查看控制台日志。

3. **登录后仍跳回登录页？**
     - token 过期/服务重启导致 token 失效，重新登录即可。
